  ...
  return (
    <div style={{ background: "rgba(0,0,0,0.3)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 14, padding: 14 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ color: "#34d399", fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" }}>
          Ocean Conditions · {index === 1 ? "Primary" : "Secondary"}
        </span>
      ...
      {/* Station */}
      <p style={{ color: "#fff", fontWeight: 600, fontSize: 12, lineHeight: 1.3, marginBottom: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{station}</p>
      <p style={{ color: "#475569", fontSize: 9, marginBottom: 14 }}>Station {stationId}</p>

      {/* Big wave height + pills row */}
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 4 }}>
          <span style={{ color: "#34d399", fontWeight: 900, fontSize: 48, lineHeight: 1 }}>{height}</span>
          <span style={{ color: "#34d399", fontWeight: 700, fontSize: 16, marginBottom: 7 }}>ft</span>
        </div>

        {/* Period + Direction pills stacked */}
        <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-end" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 20, padding: "5px 12px" }}>
            ...
            <span style={{ color: "#34d399", fontSize: 14, fontWeight: 800 }}>{period}</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 20, padding: "5px 12px" }}>
            <Navigation size={9} color="#64748b" />
            <span style={{ color: "#64748b", fontSize: 9, fontWeight: 600 }}>DIR</span>
            <span style={{ color: "#34d399", fontSize: 14, fontWeight: 800 }}>{direction}</span>
          </div>
        </div>
      </div>

      {showHistory && (
        <button style={{ marginTop: 14, width: "100%", background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.3)", borderRadius: 8, padding: "7px 0", color: "#34d399", fontSize: 10, fontWeight: 600, cursor: "pointer" }}>
          Wave History
        </button>
      )}
    </div>
