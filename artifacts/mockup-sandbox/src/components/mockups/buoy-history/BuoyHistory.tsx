      ...
      <div className="w-[600px] rounded-2xl overflow-hidden"
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="bg-emerald-900/60 text-emerald-400 text-[9px] font-semibold px-1.5 py-0.5 rounded-full border border-emerald-700/40">NOAA Live</span>
              </div>
            ...
            <div className="text-right">
              <div className="flex items-baseline gap-1 justify-end">
                <span className="text-emerald-400 font-black text-3xl leading-none">{nowData.waveHeight.toFixed(1)}</span>
        ...
        <div className="px-4 pt-4">
          ...
          <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: "visible" }}>
            <defs>
              ...
              <clipPath id="pastClip"><rect x={0} y={0} width={xOf(NOW_IDX)} height={H} /></clipPath>
              <clipPath id="futureClip"><rect x={xOf(NOW_IDX)} y={0} width={W} height={H} /></clipPath>
            </defs>
            {gridLines.map(v => (
              <g key={v}>
            ...
            <path d={smoothPath(points)} fill="none" stroke="url(#lineGrad)" strokeWidth="2" strokeLinecap="round" clipPath="url(#pastClip)" />
            <path d={smoothPath(points)} fill="none" stroke="#6ee7b7" strokeWidth="1.5" strokeLinecap="round" strokeDasharray="4 4" clipPath="url(#futureClip)" opacity="0.35" />
            {MOCK_DATA.filter((_, i) => i % 4 === 0).map((_, rawI) => {
              const i = rawI * 4;
              const [cx, cy] = points[i];
              const isPast = i <= NOW_IDX;
              return <circle key={i} cx={cx} cy={cy} r="3" fill={isPast ? "#10b981" : "#1f2937"} stroke={isPast ? "#059669" : "#374151"} strokeWidth="1.5" />;
            })}
            <line x1={nowPt[0]} y1={PAD_T} x2={nowPt[0]} y2={PAD_T + CHART_H} stroke="#10b981" strokeWidth="1.5" strokeDasharray="3 3" opacity="0.7" />
            <circle cx={nowPt[0]} cy={nowPt[1]} r="5" fill="#10b981" stroke="#022c22" strokeWidth="2" />
            <circle cx={nowPt[0]} cy={nowPt[1]} r="9" fill="none" stroke="#10b981" strokeWidth="1" opacity="0.3" />
            <text x={nowPt[0]} y={PAD_T - 4} textAnchor="middle" fill="#10b981" fontSize="8" fontFamily="sans-serif" fontWeight="700">NOW</text>
            {xLabels.map(i => (
            ...
            {MOCK_DATA.map((d, i) => {
              ...
              return (
                <div key={i} className="flex-1">
                  <div className="w-full rounded text-center text-[8px] font-semibold py-0.5" style={{
            ...
            {[0, 4, 8, 12, 16, 20, 23].map(i => {
              ...
              return (
                <div key={i} className="flex flex-col items-center gap-0.5">
                  <span className="text-[9px] text-slate-500">{i === 0 ? "12a" : i === 12 ? "12p" : i < 12 ? `${i}a` : `${i - 12}p`}</span>
        ...
        <div className="px-4 pt-3 pb-4">
          ...
          <div className="rounded-xl overflow-hidden border border-white/6">
            {/* Table header */}
            <div className="grid grid-cols-4 px-3 py-2" style={{ background: "rgba(16,185,129,0.08)" }}>
              {["Time", "Height", "Period", "Direction"].map(h => (
                <span key={h} className="text-emerald-600 text-[9px] font-bold uppercase tracking-wider">{h}</span>
              ))}
            </div>
            {/* Rows */}
            <div className="max-h-48 overflow-y-auto" style={{ scrollbarWidth: "thin", scrollbarColor: "#1e3a2a transparent" }}>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      <span className={`text-[10px] font-medium ${isNow ? "text-emerald-300" : isPast ? "text-slate-400" : "text-slate-600"}`}>
                        {d.time}
                      </span>
                      {isNow && (
                        <span className="text-[7px] bg-emerald-500 text-black font-bold px-1 py-0.5 rounded-full leading-none">NOW</span>
                      )}
                    </div>
                    <span className={`text-[10px] font-bold ${isNow ? "text-emerald-400" : isPast ? "text-emerald-500" : "text-slate-600"}`}>
                      {d.waveHeight.toFixed(1)} ft
                    </span>
                    <span className={`text-[10px] ${isNow ? "text-sky-400" : isPast ? "text-slate-400" : "text-slate-600"}`}>
                      {d.wavePeriod}s
                    </span>
                    <span className={`text-[10px] ${isNow ? "text-teal-400" : isPast ? "text-slate-400" : "text-slate-600"}`}>
                      {d.waveDirection}
                    </span>
                  </div>
                );
