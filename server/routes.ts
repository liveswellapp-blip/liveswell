...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/auth/user', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Fetch Clerk user to get their canonical email/name.
      const clerkUser = await clerkClient.users.getUser(clerkId);
      const email = clerkUser.emailAddresses[0]?.emailAddress ?? null;

      // Identity reconciliation: if no local user exists for this Clerk ID yet,
      // check whether a legacy user row (old Replit Auth UUID / numeric ID) owns
      // the same email.  If found, atomically migrate all their FK-linked data
      // (favorites, alerts, profiles, etc.) to the new Clerk ID so nothing is lost.
      const existingClerkUser = await storage.getUser(clerkId);
      if (!existingClerkUser && email) {
        const legacyUser = await storage.getUserByEmail(email);
        if (legacyUser && legacyUser.id !== clerkId) {
          console.log(`[auth] Migrating legacy user ${legacyUser.id} → Clerk ID ${clerkId} (email: ${email})`);
          await storage.migrateUserToClerkId(legacyUser.id, clerkId);
        }
      }

      // Upsert with Clerk ID (safe — any email conflict was resolved above).
...
Do not discuss topics unrelated to surfing or ocean activities.`;
  ...
  app.post("/api/support/contact", async (req, res) => {
    try {
      ...
      const ok = await EmailService.sendSupportContact(name, email, subject, message);
      if (!ok) {
